...
class ImageSelector extends StatefulWidget {
  ...
  const ImageSelector({ Key key, this.imageRepository }): super(key: key);
...
class ImageSelectorState extends State<ImageSelector> with AutomaticKeepAliveClientMixin<ImageSelector> {
  ...
  void _imageUploadProcess(SourceType source, int index) async {
    ...
    Http.StreamedResponse uploadResponse = await widget.imageRepository.uploadImage(compressedPicture); //upload the image on server
  ...
  void _deleteImageProcess(BuildContext context, int index) async{
    var response = await widget.imageRepository.deleteImage(_imagesManager.get(index));//First, delete the image from the server
